use super::types::{BodyType, Contact, ContactManifold, ManifoldPoint, RigidBody};
...
// ============================================================================
// Contact Manifold Solver (TGS Soft Phase 1)
// ============================================================================
...
/// Update contact point penetrations analytically using stored local anchors.
/// This avoids re-running narrowphase during sub-steps.
pub fn update_manifolds_analytically(
    bodies: &[Option<RigidBody>],
    manifolds: &mut [ContactManifold],
) {
    for manifold in manifolds.iter_mut() {
        ...
        let (xa, ya, cos_a, sin_a) = match &bodies[id_a] {
            Some(a) => (a.x, a.y, a.angle.cos(), a.angle.sin()),
            None => continue,
        };
        let (xb, yb, cos_b, sin_b) = match &bodies[id_b] {
            Some(b) => (b.x, b.y, b.angle.cos(), b.angle.sin()),
            None => continue,
        };
        ...
        for point in &mut manifold.points {
            ...
            // Separation along normal = dot(wb - wa, normal)
            // Penetration = -separation
            let dx = wb_x - wa_x;
            let dy = wb_y - wa_y;
            let separation = dx * nx + dy * ny;
