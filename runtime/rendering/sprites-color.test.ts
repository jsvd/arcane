...
import { drawSprite, _resetColorTexCache } from "./sprites.ts";
import {
...
describe("drawSprite with color", () => {
  ...
  it("should cache textures for same color", () => {
    enableDrawCallCapture();
    _resetColorTexCache();
    drawSprite({ color: { r: 0.5, g: 0.5, b: 0.5, a: 1 }, x: 0, y: 0, w: 8, h: 8 });
    drawSprite({ color: { r: 0.5, g: 0.5, b: 0.5, a: 1 }, x: 10, y: 10, w: 8, h: 8 });
    assert.ok(true, "cached texture reuse works without error");
    disableDrawCallCapture();
  });

  it("textureId takes priority over color", () => {
    enableDrawCallCapture();
    clearDrawCalls();
    drawSprite({
      textureId: 42,
    ...
    assert.equal((log[0] as any).textureId, 42);
    disableDrawCallCapture();
  });

  it("no-ops when neither textureId nor color provided", () => {
    enableDrawCallCapture();
    clearDrawCalls();
    drawSprite({ x: 0, y: 0, w: 16, h: 16 });
    const log = getDrawCalls();
    assert.equal(log.length, 0, "should not log a draw call with no texture or color");
    disableDrawCallCapture();
  });
});
