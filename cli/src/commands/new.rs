...
/// Try to find the runtime directory on the filesystem (for dev-from-repo).
pub(crate) fn find_runtime_dir() -> Option<PathBuf> {
    // Walk up from executable location
    if let Ok(exe) = std::env::current_exe() {
        let mut dir = exe.parent().map(|p| p.to_path_buf());
        while let Some(d) = dir {
            let candidate = d.join("runtime");
...
pub(crate) fn copy_runtime_fs(src: &Path, dst: &Path) -> Result<()> {
    ...
    for entry in fs::read_dir(src)? {
        ...
        if src_path.is_dir() {
            copy_runtime_fs(&src_path, &dst_path)?;
        } else {
            // Skip test files
...
/// Copy embedded directory to filesystem without template substitution.
pub(crate) fn copy_embedded_raw(dir: &Dir<'_>, dst: &Path) -> Result<()> {
    fs::create_dir_all(dst)?;
    for file in dir.files() {
